<template>
<!--<svg> 标签就代表 SVG 元素。SVG（Scalable Vector Graphics）是一种用于描述二维矢量图形的 XML 格式。
在 HTML 中，<svg> 元素用于创建矢量图形，可以包含各种形状、文本、图像等，
并且可以通过 CSS 和 JavaScript 进行样式和交互控制。-->
  <svg
    id="svgContent"
  ...
  >
<!--绑定了 mousedown 事件，当鼠标在 SVG 元素上按下时会触发 svgMouseDown 方法。-->
<!--绑定了 mousemove 事件，当鼠标在 SVG 元素上移动时会触发 dragIng 方法。-->
<!--绑定了 mouseleave 事件，当鼠标移出 SVG 元素时会触发 atMouseOut 方法。-->
<!--绑定了 mouseup 事件，当鼠标在 SVG 元素上抬起时会触发 dragEnd 方法。-->
    <g :transform="` translate(${svg_left}, ${svg_top}) scale(${svgScale})`">
      <!-- 节点主体 遍历DataAll中的nodes来渲染节点-->
      <g
...
<!--        <ConvForm :conv-arg="item.arg" :i="i" ref="convForm" class="form" style="position: absolute"></ConvForm>-->
      </g>
...
import hyperParameterDialog from "../FormAndDialog/hyperParameterDialog.vue";
...
export default {
  ...
  methods: {
     settingArg(id) {
         this.$emit('updateDAG', this.DataAll, 'settingArg', id)
     },
    startActive() {
      // 激活图像状态变更
      let step = this.step
